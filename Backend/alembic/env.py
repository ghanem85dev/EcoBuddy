# alembic/env.py
from logging.config import fileConfig
from sqlalchemy import engine_from_config, pool
from alembic import context
import sys
import os

# Ajoutez le chemin du projet au PYTHONPATH
sys.path.append(os.getcwd())

# Importez vos modèles SQLAlchemy ici
from app.models.models import Base  # Importez Base depuis votre fichier base.py

# Charger la configuration d'Alembic
config = context.config

# Configurer le logger
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

# Définir l'objet target_metadata pour Alembic
target_metadata = Base.metadata  # Utilisez Base.metadata ici

def run_migrations_offline():
    """Exécute les migrations en mode hors ligne."""
    url = config.get_main_option("sqlalchemy.url")
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )
    with context.begin_transaction():
        context.run_migrations()

def run_migrations_online():
    """Exécute les migrations en mode connecté."""
    connectable = engine_from_config(
        config.get_section(config.config_ini_section),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )
    with connectable.connect() as connection:
        context.configure(
            connection=connection, target_metadata=target_metadata
        )
        with context.begin_transaction():
            context.run_migrations()

if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()